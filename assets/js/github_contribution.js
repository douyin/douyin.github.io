        ...
        */
        var processListTimeStamp = function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                var _d = new Date(Date.parse(data[i].date.replace(/-/g, "/")));
                var display_date = getDisplayDate(_d);
                obj_count[display_date] = data[i].count;
                obj_text[display_date] = data[i].text;
            }
        }
        ...
        var getCount = function(display_date) {
            if (obj_count[display_date]) {
                return obj_count[display_date];
            }
            return 0;
        }
        var getText = function(display_date) {
            if (obj_text[display_date]) {
                return obj_text[display_date];
            }
            return '';
        }
        ...
        var start = function() {
            ...
            for (var i = 0; i < 56; i++) {
                ...
                for (var j = 0; j < 7; j++) {
                    ...
                    var text = getText(data_date);
                    var color = getColor(count);
                    item_html += '<rect class="day" width="11" height="11" y="' + y + '" fill="' + color + '" data-count="' + count + '" data-text="' + text + '" data-date="' + data_date + '"/>';
                }
                item_html += "</g>";
                loop_html += item_html;
            }
            ...
            for (var i = 0; i < month_position.length; i++) {
                ...
                loop_html += '<text x="' + item.x + '" y="-5" class="month" style="font-size: 12px;">' + month_name + '</text>';
            }
            //Add Monday, Wenesday, Friday label
            loop_html += '<text text-anchor="middle" class="wday" dx="-10" dy="22" style="font-size: 12px;">{0}</text>'.formatString(settings.h_days[0]) + '<text text-anchor="middle" class="wday" dx="-10" dy="48" style="font-size: 12px;">{0}</text>'.formatString(settings.h_days[1]) + '<text text-anchor="middle" class="wday" dx="-10" dy="74" style="font-size: 12px;">{0}</text>'.formatString(settings.h_days[2]);
            //Fixed size for now with width= 721 and height = 110
            var wire_html = '<svg width="721" height="110" class="js-calendar-graph-svg">' + '<g transform="translate(20, 20)">' + loop_html + '</g>' + '</svg>';
        ...
        var mouseEnter = function(evt) {
            ...
            var text = $(evt.target).attr('data-text');
            var date = $(evt.target).attr('data-date');
            var text = "{0} {1}".formatString(date, text);
